...
ASTEROID_SIZES = {3: 40, 2: 24, 1: 12}  # size_level -> radius
START_ASTEROIDS = 20
...
class Ship:
    def __init__(self):
        ...
        self.vel = [0.0, 0.0]
        self.angle = 90.0  # facing up (degrees)
        self.radius = 12
    ...
    def draw(self, surf):
        ...
        if self.invulnerable and (int(pygame.time.get_ticks() / 120) % 2 == 0):
            color = YELLOW
        else:
            color = PINK
...
class Game:
    ...
    def update(self, dt):
        ...
        # if all asteroids destroyed, spawn next wave with one extra
    ...
    def run(self):
        running = True
        rot_dir = 0  # -1 left, +1 right, 0 none
        while running:
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.key in (pygame.K_LEFT, pygame.K_a):
                        ...
                        if self.state == 'PLAYING':
                            ...
                            b = Bullet(spawn, self.ship.angle, self.ship.vel)
